...
class AttResNet(nn.Module):
    ...
    Args:
        depth (int): Depth of AttResNet, from {18, 34, 50, 101, 152}.
        stem_channels (int | None): Number of stem channels. If not specified,
            it will be the same as `base_channels`. Default: None.
        base_channels (int): Number of base channels of res layer. Default: 64.
        in_channels (int): Number of input image channels. Default: 3.
        num_stages (int): AttResNet stages. Default: 4.
    ...
    Example:
        >>> from mmdet.models import AttResNet
        >>> import torch
        >>> self = AttResNet(depth=18)
        >>> self.eval()
    ...
    def __init__(self,
        ...
        if depth not in self.arch_settings:
            raise KeyError(f'invalid depth {depth} for AttResNet')
        self.depth = depth
        ...
        self.modulatedAttLayer = ModulatedAttLayer(in_channels=2048)
    ...
    def make_stage_plugins(self, plugins, stage_idx):
        """Make plugins for AttResNet ``stage_idx`` th stage.

        Currently we support to insert ``context_block``,
        ``empirical_attention_block``, ``nonlocal_block`` into the backbone
        like AttResNet/ResNeXt. They could be inserted after conv1/conv2/conv3 of
        Bottleneck.
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(AttResNet, self).train(mode)
        self._freeze_stages()
...
class AttResNetV1d(AttResNet):
    ...
    def __init__(self, **kwargs):
        super(AttResNetV1d, self).__init__(
